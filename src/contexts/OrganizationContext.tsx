...
export function OrganizationProvider({ children }: { children: ReactNode }) {
  ...
  const fetchOrganizations = useCallback(async () => {
    ...
    try {
      ...
      // If error or no memberships, user needs onboarding
      if (memberError || !memberships || memberships.length === 0) {
        console.log('No organization memberships found, showing onboarding', memberError);
      ...
      if (orgsError || !orgsData || orgsData.length === 0) {
        console.log('No organizations found, showing onboarding', orgsError);
        setNeedsOnboarding(true);
        setOrganizations([]);
        setOrganization(null);
        setMembership(null);
      ...
      // On any error, assume user needs onboarding
      setNeedsOnboarding(true);
    } finally {
      setLoading(false);
    }
